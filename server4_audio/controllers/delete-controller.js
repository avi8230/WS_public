...
router.delete("/audio/:folderName/:fileName", async (request, response) => {
    try {
        ...
        try {
            ...
            const err = await deleteLogic.deleteAudioAsync(folderName, fileName);
            ...
            if (!err) {
                ...
                response.status(404).send(`The audio does not exist. ${err}`);
            }
